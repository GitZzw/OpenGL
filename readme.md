...
public:
	...
	void update() {
		...
		if (y > maximumHeight) {
			...
			y = radius;
			direction = 1;
			maximumHeight = maximumHeight * 0.9;
		}
...
//全局变量：棋盘格、相机和3个球的数组
Checkerboard checkerboard(60, 60);
Camera camera;
//创建3个小球的数组
Ball balls[] = {
	//Ball(1, GREEN, 5, 6, 1),
	//Ball(1.5, MAGENTA, 5, 3, 4),
	Ball(0.4, WHITE, 5, 1, 30)
};
...
void display() {
	...
	gluLookAt(camera.getX()+20, camera.getY(), camera.getZ(),    //摄像机位置
		gx, 2.5, gz,   //焦点坐标
		0.0, 1.0, 0.0);   //摄像机机顶方向矢量
						  //绘制棋盘
	checkerboard.draw();
